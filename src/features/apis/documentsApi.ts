...
export const documentsApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    renderDocument: builder.mutation<{ success: boolean; data: { downloadUrl: string; format: string } }, { meetingId: string; data: { template_id?: string; format: string; version?: number; document_number?: string; scope?: 'all' | 'staff' | 'visitors' } }>({
