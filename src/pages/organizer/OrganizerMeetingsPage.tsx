...
export const OrganizerMeetingsPage: React.FC<OrganizerMeetingsPageProps> = ({
  ...
  // Mutations
  const [openAttendance] = useOpenMeetingAttendanceMutation();
  const [closeAttendance] = useCloseMeetingAttendanceMutation();
  ...
  const toggleMeetingStatus = async (m: any) => {
    const isCurrentlyOpen = m.attendance_status === 'open';
    try {
      if (isCurrentlyOpen) {
        await closeAttendance(m.meeting_id).unwrap();
        showToast('Attendance register is now closed.');
      } else {
        await openAttendance(m.meeting_id).unwrap();
        showToast('Attendance register is now open.');
      }
    } catch (err: any) {
  ...
  const handleSubmitToHR = async (m: any) => {
    try {
      ...
      showToast('Report submitted successfully to the Human Resource repository');
    } catch (err: any) {
          ...
          <div className="panel-header" style={{ flexWrap: 'wrap', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }} className="btn-group-no-print">
              ...
              {selectedMeeting.attendance_status === 'open' && (
                <button type="button" onClick={() => toggleMeetingStatus(selectedMeeting)} className="btn btn-danger">
                  Close Attendance Register
                </button>
              )}
              {selectedMeeting.attendance_status === 'not_started' && (
                <button type="button" onClick={() => toggleMeetingStatus(selectedMeeting)} className="btn btn-success">
